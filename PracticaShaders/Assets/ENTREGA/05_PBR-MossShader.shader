Shader "ENTI/05_PBR-MossShader"
{
    Properties
    {
        ...
        [Header(Triplanar Things)]
        _TopFaceTex("Top Face Texture", 2D) = "white" {}
        _SideFaceTex("Side Face Texture", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _TopFaceTex;
            float4 _TopFaceTex_ST;
            sampler2D _SideFaceTex;
            float4 _SideFaceTex_ST;
            float _Sharpness;
            ...
            float4 frag(v2f i) : SV_TARGET
            {
                ...
                //READ MAPS------------------------------------------------------------------------------------------
                //1. Triplanar Faces
                float2 uv_front = TRANSFORM_TEX(i.worldPos.yz, _SideFaceTex);
                float2 uv_top = TRANSFORM_TEX(i.worldPos.xz, _TopFaceTex);
                float2 uv_side = TRANSFORM_TEX(i.worldPos.xy, _SideFaceTex);

                fixed4 col_front = tex2D(_SideFaceTex, uv_front);
                fixed4 col_top = tex2D(_TopFaceTex, uv_top);
                fixed4 col_side = tex2D(_SideFaceTex, uv_side);
                ...
                normalDirection = lerp(normalDirection, worldNormal, _NormalStrength);
                ...
                if (i.normal.y < 0.0)
                {
                    uv_top = TRANSFORM_TEX(i.worldPos.xz, _SideFaceTex);
                    col_top = tex2D(_SideFaceTex, uv_top);
                }
                ...
                i.col.rgb = (col_front + col_top + col_side) * _Exposure;
                i.col.a = 1.0;
                ...
                //float3 lightingModel = col_front + col_top + col_side + diffuseColor + indirectDiffuse + specularity + indirectSpecular;
